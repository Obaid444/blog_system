        ...
        <form action="{{ route('posts.update', $post) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="content" rows="6"
                          class="mt-1 w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200 focus:border-blue-500 px-3 py-2">{{ old('content', $post->content) }}</textarea>
                @error('content')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
